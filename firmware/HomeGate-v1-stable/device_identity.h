...
inline void deviceLoadIdentity() {
  ...
  if (strlen(FACTORY_PRODUCT_ID) > 0) {
    // Home / factory flash: always use compile-time product (overrides SoftAP mistakes).
    deviceProductIdRef() = FACTORY_PRODUCT_ID;
    deviceProductSecretRef() = FACTORY_PRODUCT_SECRET;
    Preferences &w = devicePrefs();
    w.begin("homegate", false);
    w.putString("productId", deviceProductIdRef());
    w.putString("productSecret", deviceProductSecretRef());
    w.end();
  } else if (deviceProductIdRef().length() == 0) {
    /* wait for app claim / SoftAP bind */
  }
